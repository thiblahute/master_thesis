...
filtre et consommateur.

On peu schématiser ce concept de la manière suivante:
...
Afin de coordonner le tout, les devellopeurs on créés une classe
``service'' qui contient les différents élément et sur laquelle on
connect directement les différents consommateurs, ce qui permet au
final de créer des timelines, avec mes outputs voulus.  On peu donc
schématiser les services de la manière suivante:
...
Au niveau de l'accélération materiel, MLT supporte le decoding vidéo
pour les cartes graphique Nvidia

\glossary{name={Nvidia}, description={Nvidia Corporation est l'un des
plus grands fournisseurs de processeurs graphiques, de cartes graphiques
et de chipsets pour PC et consoles de jeux}}

(via VDPAU), et pour l'affichage mais pas le compositing vidéo accéléré.

\glossary{name={VDPAU}, description={VDPAU (Video Decode and
Presentation API for Unix) est une bibliothèque open source (libvdpau)
et une interface de programmation conçus par NVIDIA initialement pour ses
