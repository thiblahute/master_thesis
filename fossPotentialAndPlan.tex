...
faire le montage de manière collaborative %avec Internet, en
...
pour principal avantage %de présenter une conception % simplifiée pour %les raisons suivantes:
...
un core qui ressemble fortement à un framework \index{framework}).%dans le monde des logiciels libres,les développeures de Cinelerra ont décidé d'utiliser ce mode de
fonctionnement.
...
\begin{itemize} \setlength{\itemsep}{2mm}
  ...
  \item {Les développeurs n'ont besoin de penser l'architecture %seulement pour
    les % cas d'utilisation qui sont liés à ce même logiciel:
    ils n'ont pas à voir au delà de ces %???je ne comprends pas "uses cases".}
...
%pas compter sur d'autre utilisation de ce code pour améliorer et développer le core
du logiciel.

\paragraph{Réutilisabilité:%néologisme qui n'existe pas en français}
...
plus abouti%,bien qu'il y ait évidemment de nombreux autres facteurs qui interviennent
%en particulier le fait que ce logiciel a été développé par la société
Heroine Virtual.

\subsubsection {Utilisation de  frameworks \index{framework}}

\paragraph{} L'autre possibilité est de séparer en deux parties%
...
\begin{itemize}
  ...
  \item{ffmpeg: Solution compete, cross plateforme d'enregistrement, lecture, conversion
    de flux audio et vidéo. Il inclue libavcodec, librairie leader dans le domaines des codec.
